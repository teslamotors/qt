...
#else
unsigned char * q_ASN1_STRING_get0_data(ASN1_STRING *a);
#endif
...
void q_ENGINE_load_builtin_engines(void);
ENGINE *q_ENGINE_by_id(const char *id);
int q_ENGINE_init(ENGINE* e);
int q_ENGINE_init(ENGINE *e);
EVP_PKEY *q_ENGINE_load_private_key(ENGINE *e, const char *key_id,
                                    UI_METHOD *ui_method, void *callback_data);
EVP_PKEY *q_ENGINE_load_public_key(ENGINE *e, const char *key_id,
                                   UI_METHOD *ui_method, void *callback_data);
int q_ENGINE_finish(ENGINE *e);
int q_ENGINE_free(ENGINE *e);
...
char *q_X509_NAME_oneline(const X509_NAME *a, char *buf, int size);
...
long q_SSLeay();
#else
#define q_SSLeay() OPENSSL_VERSION_NUMBER
#endif

#if OPENSSL_VERSION_NUMBER >= 0x10100000L
void q_DSA_get0_pqg(const DSA *d, const BIGNUM **p, const BIGNUM **q, const BIGNUM **g);
void q_RSA_get0_key(const RSA *r, const BIGNUM **n, const BIGNUM **e, const BIGNUM **d);
int q_SSL_CIPHER_get_bits(const SSL_CIPHER *cipher, int *alg_bits);
long q_X509_get_version(const X509 *x);
ASN1_INTEGER *q_X509_get_serialNumber(X509 *x);
EVP_PKEY *q_X509_get0_pubkey(const X509 *x);
